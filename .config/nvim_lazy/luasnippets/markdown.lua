...
local check_tex_is_math = require("snippet.utils").in_markdown_mathzone

local check_tex_is_not_math = function()
  return not check_tex_is_math()
end

local function make_markdown_snippet(snip_fn)
  return snip_fn(check_tex_is_math, check_tex_is_not_math)
end

return concat(map(unflattened, make_markdown_snippet))
